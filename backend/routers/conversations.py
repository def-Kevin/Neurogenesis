...
def send_message_stream(
    ...
    _convo_id = convo.id
    _session_key = convo.openclaw_session_key

    # Route avatar conversations through OpenClaw Gateway when available
    use_gateway = bool(_avatar_id and gateway_client.connected)
    ...
    if use_gateway:
        ...
        async def _gateway_stream():
            ...
            try:
                ...
                if not session_key:
                    agent_id = f"neurogenesis_avatar_{_avatar_id}"
                    session_key = await gateway_client.create_session(agent_id)
                    sk_db = SessionLocal()
                    try:
                        c = sk_db.query(models.Conversation).filter(models.Conversation.id == _convo_id).first()
                        if c:
                            c.openclaw_session_key = session_key
                            sk_db.commit()
                    finally:
                        sk_db.close()

                run_id = await gateway_client.send_message(session_key, data.content)
                async for chunk in gateway_client.stream_run(run_id):
                    full_content += chunk
                    yield f"data: {json.dumps({'chunk': chunk}, ensure_ascii=False)}\n\n"

            except Exception as e:
                import traceback
                print(f"[OpenClaw] Gateway stream error: {e}")
                traceback.print_exc()
                full_content = "抱歉，我暂时有点懵，稍后再试好吗？"
                yield f"data: {json.dumps({'chunk': full_content}, ensure_ascii=False)}\n\n"

            # Cleanup outside try/finally to allow yield after
            p_db = SessionLocal()
            try:
                p_db.add(models.Message(
                ...
                print(f"[OpenClaw] Failed to persist message: {e}")
            finally:
                p_db.close()
            if _avatar_id:
                threading.Thread(
                    target=_extract_memory_bg,
                    args=(_avatar_id, _convo_id),
                    daemon=True,
                ).start()
            yield f"data: {json.dumps({'done': True}, ensure_ascii=False)}\n\n"

        from fastapi.responses import StreamingResponse
        return StreamingResponse(_gateway_stream(), media_type="text/event-stream")

    # Fallback: direct LLM via agent_engine (non-avatar chats, or when gateway unavailable)
    def event_generator():
        full_content = ""
        try:
            from backend.database import SessionLocal
            ...
            if _avatar_id:
                threading.Thread(
                    target=_extract_memory_bg,
                    args=(_avatar_id, conversation_id),
                    daemon=True,
                ).start()
