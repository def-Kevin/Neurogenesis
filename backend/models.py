...
class Conversation(Base):
    ...
    user_id = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    avatar_id = Column(Integer, ForeignKey("avatars.id"), nullable=True)
    conversation_type = Column(String, default="ai")
    title = Column(String)
    status = Column(String, default="active")
    openclaw_session_key = Column(String, nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
...
class ConversationParticipant(Base):
    ...
    __table_args__ = (UniqueConstraint("conversation_id", "user_id", name="uix_convo_participant"),)
...
class AvatarSubAgent(Base):
    __tablename__ = "avatar_sub_agents"
    id = Column(Integer, primary_key=True, index=True)
    parent_avatar_id = Column(Integer, ForeignKey("avatars.id", ondelete="CASCADE"), nullable=False)
    name = Column(String, nullable=False)
    task = Column(Text)
...
class AgentMessage(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    sender_avatar_id = Column(Integer, ForeignKey("avatars.id"), nullable=False)
    receiver_avatar_id = Column(Integer, ForeignKey("avatars.id"), nullable=False)
    content = Column(Text, nullable=False)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
